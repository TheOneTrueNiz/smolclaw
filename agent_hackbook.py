...
SUMMARIZE_PROMPT = """Rewrite conversation notes incorporating new exchanges.
...
def call_llm(messages: list, max_tokens: int = None, temperature: float = None,
    ...
    Pass grammar= for GBNF grammar-constrained decoding (forces structured output)."""
    body = {
        "model": MODEL,
        "messages": messages,
    ...
    if grammar:
        # Grammar mode: don't send tools (grammar forces structure)
